...
class BNSSParser:
    ...
    def _parse_body(body_text: str, toc_entries: list[dict]) -> list[Section]:
        ...
        for pos, (i, match) in enumerate(matched_positions):
            ...
            # own trailing line (e.g. section 531's body ends "...for the
            # \nextension of time.\n172") - remove_repeated_headers can't
            # catch these since each page number is different text, not
            # a repeated string. confirmed this affects 34 sections
            # across the document, not just the one before
            # BODY_END_MARKER. strips only a trailing STANDALONE
            # digit-only line (preceded by a newline) - never digits
            # that are part of the last real sentence, since those
